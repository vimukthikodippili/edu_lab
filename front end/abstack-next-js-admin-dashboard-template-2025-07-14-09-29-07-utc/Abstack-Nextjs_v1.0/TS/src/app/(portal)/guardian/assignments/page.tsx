...
import { ListChecks, CheckCircle, Clock, AlertTriangle, HelpCircle, Calendar } from 'lucide-react'
...
function GuardianAssignmentsContent() {
  ...
  const { data: rows, isLoading: rowsLoading } = useChildAssignments(studentId)
    ...
    <div className="container-fluid py-4" style={{ maxWidth: 800 }}>
      <div className="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-muted small mb-0">Your child&apos;s assignment status and grades.</p>
        </div>
      </div>
      ...
      {rowsLoading ? (
        ...
        <div className="card border-0 shadow-sm rounded-4">
          <div className="card-body text-center text-muted py-5">
            <ListChecks size={36} className="mb-3 opacity-25" />
            <p className="mb-0">No assignments for this child&apos;s class yet.</p>
          </div>
        </div>
      ) : (
