...
describe('DomainResultNoteCryptoService', () => {
  let service: DomainResultNoteCryptoService;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DomainResultNoteCryptoService,
        {
    ...
    service = module.get<DomainResultNoteCryptoService>(DomainResultNoteCryptoService);
  });

  it('round-trips: decrypting an encrypted plaintext returns the original plaintext', () => {
    const plaintext = 'Withdrawn during group activities, worth a follow-up check-in.';
  ...
  it('never stores the plaintext as-is — ciphertext differs from the original text', () => {
    const plaintext = 'Domain notes must never be stored in the clear.';
